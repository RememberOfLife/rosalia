...
ROSALIA__BASE64_DEC_EXT const char b64_chars[64];

ROSALIA__BASE64_DEC_EXT const char b64_inv[80]; // = 'z' - '+' + 1

// returns the size (including null character) of the char buffer required to encode this many bytes
ROSALIA__BASE64_DEC size_t b64_encode_size(size_t size_bytes);

// returns the size (byte count) produced by decoding this char buffer
ROSALIA__BASE64_DEC size_t b64_decode_size(const char* data_chars);

// encode data_bytes as base64 into data_chars and returns the number of bytes written
ROSALIA__BASE64_DEC size_t b64_encode(char* data_chars, const uint8_t* data_bytes, size_t len);

// decode data_chars base64 into data_bytes and returns the number of bytes decoded
ROSALIA__BASE64_DEC size_t b64_decode(uint8_t* data_bytes, const char* data_chars);
...
const char b64_chars[64] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-"; // not real base64 but much more usable
...
ROSALIA__BASE64_DEF size_t b64_encode_size(size_t size_bytes)
{
...
ROSALIA__BASE64_DEF size_t b64_decode_size(const char* data_chars)
{
    if (data_chars == NULL) {
        return 0;
    }
...
ROSALIA__BASE64_DEF size_t b64_encode(char* data_chars, const uint8_t* data_bytes, size_t len)
{
...
ROSALIA__BASE64_DEF bool b64_is_valid_char(char c)
{
    if (c >= '0' && c <= '9') {
        return 1;
    }
...
ROSALIA__BASE64_DEF size_t b64_decode(uint8_t* data_bytes, const char* data_chars)
{
